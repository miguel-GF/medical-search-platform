-- Require every meaningful query token to be represented for non-exact matches.
-- This closes the remaining case where a shared word such as "suero" could
-- make ALUMINIO appear for an AMILASA query.

begin;

create or replace function catalog.resolve_items_v3(
  p_query text,
...
with q as (
  ...
  select r.*
  from catalog.resolve_items_v2(p_query, p_domain_code, p_provider_brand_id, greatest(50, coalesce(p_limit,10) * 4)) r
), guarded as (
  select c.*,
    (
      select count(*)
  ...
  where g.match_method in ('exact','alias','disambiguation')
     or (g.meaningful_token_count > 0 and g.meaningful_token_matches = g.meaningful_token_count)
), ranked as (
  select a.*,
...
from catalog.resolve_items_v3(p_query, p_domain_code, p_provider_brand_id, p_limit) r
where r.resolution_status in ('resolved','ambiguous')
order by r.confidence desc, r.display_name
limit greatest(1, least(coalesce(p_limit,10),50));
$$;

revoke all on function catalog.resolve_items_v3(text,text,uuid,integer) from public;
grant execute on function catalog.resolve_items_v3(text,text,uuid,integer) to anon, authenticated, service_role;
