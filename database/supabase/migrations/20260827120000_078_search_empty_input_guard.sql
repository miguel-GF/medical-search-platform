-- Avoid invoking the internal search implementation for an empty normalized query.
...
with valid_input as (
  ...
  select s.*
  from valid_input v
  cross join lateral public.api_search_scoped_v1(
    p_query,
    p_domain_code,
    p_latitude,
    p_longitude,
...
select
  p.service_id, p.display_name, p.matched_term, p.term_source, p.confidence,
  p.offer_id, p.provider_brand_id, p.provider_name, p.provider_location_id,
  p.provider_location_name, p.latitude, p.longitude, p.distance_meters,
  p.source_url, p.price_type, p.price_key, p.amount_minor, p.currency,
