-- Build the single-study resolver from the typed search rows directly.  The
-- legacy resolver intentionally keeps a small diversity sample, which is not
-- enough for the patient comparison page once one offer spans many branches.
