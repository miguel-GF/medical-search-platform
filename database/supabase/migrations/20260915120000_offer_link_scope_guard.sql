-- Keep the JSON resolver aligned with the typed-link API.
--
-- 20260915110000 introduced the wrapper, but an earlier development
-- application can leave the wrapper without the concrete-location guard
-- while the migration history still reports it as pending.  This follow-up
-- is intentionally create-or-replace and is safe on both fresh and repaired
-- projects.
  ...
  select c.ordinality,
    c.value || jsonb_build_object(
      'offers', coalesce((
        ...
        where not exists (
          -- A concrete location scope wins over a legacy brand/market
          -- fallback. Never manufacture availability at another branch.
          select 1
